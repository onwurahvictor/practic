...
router.post('/contact', async (req, res) => {
  ...
  try {
    ...
    // Email notification is best-effort — a failure here must never make the
    // form look like it failed, since the enquiry is already safely saved.
    try {
      await sendEnquiryEmail({ name, business, email, phone, message });
    } catch (emailErr) {
      console.error('Enquiry email failed to send:', emailErr.message);
    }
